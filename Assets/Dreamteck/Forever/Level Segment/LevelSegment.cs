...
namespace Dreamteck.Forever
{
    ...
    public partial class LevelSegment : MonoBehaviour
    {
        ...
        [SerializeField]
        [HideInInspector]
        private UnityEngine.Object[] uniqueResources = new UnityEngine.Object[0];
        ...
        public void Setup(LevelGenerator generator, int index)
        {
            ...
            if (ResourceManagement.available)
            {
        ...
        {
            if(_generator.segments[_generator.segments.Count-1] != this)
            {
                Debug.LogError("Attempting to set segment loop but " + name + " is not the last segment");
                return;
            }
            _next = firstSegment;
        ...
        public void Destroy()
        {
            ...
            if (_parentDefinition.pooling)
            {
                _parentDefinition.ReturnToPool(gameObject);
            }
            else
            {
                Destroy(gameObject);
        ...
        public void EditorPack()
        {
            ...
            #region Extract Unique Resources
                if (allChildren[i].GetComponent<PreventFromUnloading>() == null)
                {
